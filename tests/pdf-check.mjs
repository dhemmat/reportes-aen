...
const samples = ['Transacciones.xlsx', 'Balance.xlsx']
  .map(f => path.join(ROOT, 'docs/samples', f));

if (!CHROME)                             skip('no Chrome/Chromium found');
if (!samples.every(f => fs.existsSync(f))) skip('docs/samples/ not present');
...
const boot = `<script>(function(){
  ...
  _txData=d("${b64(samples[0])}"); _balRawData=d("${b64(samples[1])}");
  loadFiles();
